...
class CurrentTicketSalesGenerator:
    ...
    def categorize_venue_type(self, venue_name):
        ...
        if any(
            word in venue_lower for word in ["stadium", "field", "dome", "coliseum"]
        ):
            return "Stadium"
        elif any(
            word in venue_lower
            for word in ["arena", "center", "pavilion", "auditorium"]
        ):
            return "Arena"
        elif any(
            word in venue_lower for word in ["theater", "theatre", "hall", "opera"]
        ):
            return "Theater"
        elif any(word in venue_lower for word in ["club", "bar", "lounge", "cafe"]):
            return "Club"
        elif any(
            word in venue_lower for word in ["amphitheater", "amphitheatre", "outdoor"]
        ):
            return "Amphitheater"
        else:
            return "Other"
    ...
    def calculate_current_ticket_sales(
        self, artist_tier, venue_capacity, market_size, show_date
    ):
        ...
        current_sales_rate = max(
            0.05, min(0.4, current_sales_rate * sales_variance)
    ...
    def generate_current_sales_data(self, concerts_df):
        ...
        logger.info(
            "Generating current ticket sales projections for future concerts..."
        )

        # Add venue categorization
        concerts_df["VENUE_TYPE"] = concerts_df["venue_name"].apply(
            self.categorize_venue_type
        )
        concerts_df["VENUE_CAPACITY"] = concerts_df.apply(
            lambda row: self.estimate_venue_capacity(
                row["venue_name"], row["VENUE_TYPE"]
            ),
            axis=1,
        )

        # Add artist tier
        concerts_df["ARTIST_TIER"] = (
            concerts_df["artist_name"].map(self.artist_tiers).fillna("B-list")
        )

        # Add market size
        concerts_df["MARKET_SIZE"] = concerts_df["city_name"].apply(
        ...
        for idx, row in concerts_df.iterrows():
            ...
            tickets_sold_so_far, current_sales_rate = (
                self.calculate_current_ticket_sales(
                    row["ARTIST_TIER"],
                    row["VENUE_CAPACITY"],
                    row["MARKET_SIZE"],
                    row["show_date"],
            ...
            sales_data.append(
                {
                    ...
                    "DAYS_UNTIL_SHOW": (
                        pd.to_datetime(row["show_date"]) - datetime.now()
                    ).days,
                }
            )
...
def main():
    ...
    try:
        ...
        logger.error(
            "real_us_future_concerts_2025_2026.csv not found. Run real_us_future_concerts_2025_2026.py first."
        )
        return
    ...
    print(
        f"Average current sales rate: {enhanced_df['CURRENT_SALES_RATE'].mean():.1f}%"
    )
    print(f"Average days until show: {enhanced_df['DAYS_UNTIL_SHOW'].mean():.0f}")

    print("\n=== BY ARTIST TIER ===")
    tier_summary = (
        enhanced_df.groupby("ARTIST_TIER")
        .agg(
            {
    ...
    print("\n=== BY VENUE TYPE ===")
    venue_summary = (
        enhanced_df.groupby("VENUE_TYPE")
        .agg(
            {
                "VENUE_CAPACITY": "mean",
