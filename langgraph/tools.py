...
def _generate_ticket_sales_query(
    ...
    if hours is not None:
        where_clauses.append(f"timestamp >= DATEADD(hour, -{hours}, CURRENT_TIMESTAMP())")
...
def analyze_query_complexity(query: str) -> dict[str, Any]:
    ...
    if 'WHERE' not in query_upper:
        is_expensive = True
        if limit_value:
            reasons.append(f"No WHERE clause - scans entire table before LIMIT {limit_value}")
        else:
    ...
    if 'WHERE' in query_upper:
        ...
        where_content = query_upper.split('WHERE')[1].split('ORDER')[0] if 'ORDER' in query_upper else query_upper.split('WHERE')[1]
        # Count meaningful filters (not just status)
        has_artist_filter = 'ARTIST_NAME' in where_content or 'ARTIST_ID' in where_content
        ...
        if has_status_filter and not has_artist_filter and not has_venue_filter:
            if limit_value and limit_value >= 50:
                is_expensive = True
                reasons.append(f"Broad query - only status filter, scans large portion of table (LIMIT {limit_value})")
    ...
    if 'WHERE' in query_upper and limit_value and limit_value >= 100:
        ...
        if 'TIMESTAMP' in query_upper or 'DATEADD' in query_upper:
            ...
            if limit_value >= 100:
                is_expensive = True
                reasons.append(f"Large LIMIT ({limit_value}) with time-based filter - may scan many rows")
...
def _execute_snowflake_query(query: str, limit: int = 100) -> str:
    ...
    try:
        ...
        import re
        query = re.sub(r'\bfact_shows\b', f'{actual_db}.{actual_schema}.fact_shows', query, flags=re.IGNORECASE)
        query = re.sub(r'\bfact_ticket_sales\b', f'{actual_db}.{actual_schema}.fact_ticket_sales', query, flags=re.IGNORECASE)
        query = re.sub(r'\bdim_artists\b', f'{actual_db}.{actual_schema}.dim_artists', query, flags=re.IGNORECASE)
        query = re.sub(r'\bdim_venues\b', f'{actual_db}.{actual_schema}.dim_venues', query, flags=re.IGNORECASE)
        ...
        error_msg += f"Available schemas: FAN_MARTS (for fact_shows, fact_ticket_sales, dim_artists, etc.)\n"
        if conn and cursor:
            try:
                cursor.execute("SELECT CURRENT_DATABASE(), CURRENT_SCHEMA()")
                db_info = cursor.fetchone()
                error_msg += f"Current database: {db_info[0]}, Current schema: {db_info[1]}\n"
                # Try to list tables in current schema
                cursor.execute(f"SHOW TABLES IN SCHEMA {db_info[0]}.{db_info[1]}")
                tables = cursor.fetchall()
                if tables:
                    table_names = [t[1] for t in tables]  # Table name is usually in second column
    ...
    try:
        ...
        for result in results.get("results", []):
            ...
            formatted_results.append({
                "relevance_score": result.get("rerank_score") or result.get("similarity_score", 0),
