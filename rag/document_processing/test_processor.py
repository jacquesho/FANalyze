...
def test_document_processing(file_path: str):
    ...
    try:
        ...
        print("   Strategy: Token-based chunking")
        print("   Chunk size: 1000 tokens")
        print("   Overlap: 200 tokens\n")
        ...
        if chunks:
            ...
            print(
                f"Chunk {first_chunk['metadata']['chunk_index'] + 1}/{first_chunk['metadata']['total_chunks']}"
            )
            print(
                f"Size: {first_chunk['metadata']['chunk_size']} chars, {first_chunk['metadata']['chunk_size_tokens']} tokens"
            )
        ...
        print(
            f"Token count range: {stats['min_token_count']} - {stats['max_token_count']} tokens"
        )
