...
3. ERROR HANDLING FALLBACK
   ...
   Result: System doesn't crash, returns structured error
    ...
    print(f"  - Top result source: {result1['results'][0]['source'].split('/')[-1] if result1['results'] else 'None'}")
    ...
    if result2['results']:
        print(f"  - Top result source: {result2['results'][0]['source'].split('/')[-1]}")
        score = result2['results'][0].get('rerank_score') or result2['results'][0].get('similarity_score') or 0
    ...
    if result3['results']:
        print(f"  - Top result similarity score: {result3['results'][0].get('similarity_score', 0):.4f}")
        print(f"  - Note: No rerank_score field (reranking was skipped)")
    ...
    if result4['results']:
        print(f"  - Top result similarity score: {result4['results'][0].get('similarity_score', 0):.4f}")
        print(f"  - Note: Results sorted by similarity score, not rerank score")
