...
def create_real_us_future_concerts_csv():
    ...
    df["show_id"] = df.apply(
        lambda row: f"real_{row['artist_name'].lower().replace(' ', '_')}_{row['show_date'].replace('-', '')}",
        axis=1,
    )
    df["venue_id"] = df.apply(
        lambda row: f"real_{row['venue_name'].lower().replace(' ', '_').replace(',', '')}",
        axis=1,
    )
    ...
    print(
        f"Date range: {df['show_date_parsed'].min()} to {df['show_date_parsed'].max()}"
    )
    ...
    print("\n=== SAMPLE DATA ===")
    print(
        df[["artist_name", "show_date", "venue_name", "city_name", "state_code"]].head(
            10
        )
    )
