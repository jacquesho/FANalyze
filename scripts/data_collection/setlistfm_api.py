...
class SetlistFMAPI:
    ...
    def make_api_request_with_retry(self, url: str, max_retries: int = None) -> requests.Response:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    console.print(f"⏳ Rate limited (429). Waiting {delay} seconds before retry {attempt + 1}/{max_retries}", style="yellow")
                    time.sleep(delay)
                    continue
                else:
                ...
                console.print(f"❌ Request failed on attempt {attempt + 1}: {str(e)}", style="red")
        ...
        return requests.get(url, headers=self.config.headers, timeout=self.config.timeout)
    
    def fetch_artist_setlists(self, artist: Artist, starting_tour: Optional[str] = None) -> List[Dict[str, Any]]:
        """Fetch all setlists for a given artist from SetlistFM API"""
        all_shows = []
        page = 1
        total_pages = None
        
        console.print(f"🎵 Fetching setlists for {artist.name} (ID: {artist.musicbrainz_id})", style="blue")
        ...
        while True:
            ...
            if total_pages is None:
                ...
                if total_pages <= 1 and total == 0:
                    console.print("⚠️ Could not determine total pages, will continue until no more data", style="yellow")
                    total_pages = float('inf')  # Continue until no more data
            
            console.print(f"✅ Fetched {len(setlists)} shows from page {page} for {artist.name}", style="green")
            console.print(f"📈 Total shows collected so far: {len(all_shows)}", style="green")
        ...
        console.print(f"🎉 {artist.name}: {len(all_shows)} shows collected total", style="bold green")
        return all_shows
    ...
    def fetch_artist_setlists_incremental(
        ...
        if last_show_date is None:
            ...
            console.print(f"📅 No previous shows found for {artist.name}, fetching last 30 days", style="yellow")
        else:
    ...
    def fetch_all_artists_historical(self, artists: Optional[List[Artist]] = None) -> Dict[str, List[Dict[str, Any]]]:
        ...
        with Progress(
            ...
            task = progress.add_task("Fetching historical data for all artists...", total=len(artists))
            ...
            for artist in artists:
                ...
                try:
                    ...
                    console.print(f"❌ Failed to fetch data for {artist.name}: {e}", style="red")
                    all_artist_data[artist.musicbrainz_id] = {
                        "artist": artist,
                        "shows": [],
                        "error": str(e)
    ...
    def save_data_to_file(self, data: Dict[str, Any], filename: str, output_dir: Path = None) -> Path:
        ...
        data_with_metadata = {
            "metadata": {
                ...
                "total_shows": sum(len(artist_data.get("shows", [])) for artist_data in data.values())
            },
            "data": data
        }
