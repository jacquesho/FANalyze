...
def create_ingest_user():
    ...
    try:
        ...
        if table_exists:
            ...
            console.print(
                f"📋 Existing table columns: {existing_columns}", style="cyan"
            )
        ...
        if table_exists:
            console.print(
                "⚠️ Table exists with different structure, dropping and recreating...",
                style="yellow",
            )
        ...
        console.print(
            "🔐 Granting permissions to user_fanalyze_ingest...", style="blue"
        )
        cursor.execute(
        ...
        console.print(
            "✅ user_fanalyze_ingest user created successfully!", style="green"
        )
        return True
...
def verify_user_creation():
    ...
    try:
        ...
        if not admin_conn:
            console.print(
                "❌ Cannot verify user - admin connection failed", style="red"
            )
            return False

        admin_cursor = admin_conn.cursor()
        admin_cursor.execute(
            "SELECT rolname, rolcanlogin FROM pg_roles WHERE rolname = 'user_fanalyze_ingest';"
        )
        ...
        if not user_info:
            console.print(
                "❌ User user_fanalyze_ingest does not exist in PostgreSQL", style="red"
            )
            return False

        console.print(
            "✅ User user_fanalyze_ingest exists in PostgreSQL", style="green"
        )
        console.print(f"📊 User can login: {user_info[1]}", style="cyan")

        if not user_info[1]:  # rolcanlogin is False
            console.print(
                "⚠️ User exists but cannot login - this may be the issue", style="yellow"
            )

        # Test connection with the new user using the exact password from creation
        console.print(
            "🔐 Testing connection with user_fanalyze_ingest...", style="blue"
        )
        conn = psycopg.connect(
        ...
        # Test table access
        cursor.execute(
            "SELECT table_name FROM information_schema.tables WHERE table_schema = 'staging' AND table_name = 'test_ingest';"
        )
