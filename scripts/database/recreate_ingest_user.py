...
def recreate_user() -> int:
    ...
    timezone = os.getenv("POSTGRES_INGEST_TIMEZONE") or os.getenv("TIMEZONE") or "Asia/Bangkok"

    target_user = "user_fanalyze_ingest"

    with admin_connect() as conn:
        with conn.cursor() as cur:
            console.print(f"🔌 Connected as admin. Recreating role '{target_user}'", style="cyan")
            ...
            if exists:
                cur.execute(sql.SQL("DROP OWNED BY {} CASCADE").format(sql.Identifier(target_user)))
                cur.execute(sql.SQL("DROP ROLE IF EXISTS {};").format(sql.Identifier(target_user)))
            else:
                cur.execute(sql.SQL("DROP ROLE IF EXISTS {};").format(sql.Identifier(target_user)))
            ...
            cur.execute(
                sql.SQL("CREATE ROLE {} WITH LOGIN PASSWORD {};").format(
                    sql.Identifier(target_user), sql.Literal(env["POSTGRES_PASSWORD_INGEST"]) 
                )
            )
            ...
            cur.execute(sql.SQL("GRANT CONNECT ON DATABASE {} TO {};").format(sql.Identifier(dbname), sql.Identifier(target_user)))
            # Schema usage and default privileges in staging
            cur.execute(sql.SQL("GRANT USAGE ON SCHEMA staging TO {};").format(sql.Identifier(target_user)))
            cur.execute(sql.SQL("ALTER DEFAULT PRIVILEGES IN SCHEMA staging GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO {};").format(sql.Identifier(target_user)))
