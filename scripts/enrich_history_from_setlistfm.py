...
def price_for(tier: str, venue_type: str) -> int:
    base = BASE_PRICE.get(tier, BASE_PRICE["B-list"]).get(
        venue_type, BASE_PRICE["B-list"]["Other"]
    )
...
def enrich_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["VENUE_CAPACITY"] = df.apply(
        lambda r: estimate_capacity(r["VENUE_NAME"], r["VENUE_TYPE"]), axis=1
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Enrich historical shows CSV with synthetic ticket metrics (in-place)."
    )
    parser.add_argument(
        "--input",
        default="data/raw/csv/shows_history.csv",
        help="Path to shows_history.csv",
    )
    parser.add_argument(
        "--backup", action="store_true", help="Write a .bak copy before overwriting"
    )
