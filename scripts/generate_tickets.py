...
def generate_ticket_sales(show_row):
    ...
    print(
        f"  Venue capacity: {venue_capacity}, Current tickets sold: {current_tickets_sold}"
    )
    ...
    for days_ago in range(1, days_to_simulate + 1):
        ...
        sales_velocity = calculate_sales_velocity(
            show_date, artist_tier, venue_capacity, days_until_show_at_sale_time
        )
        ...
        max_possible_sales = min(
            int(venue_capacity * actual_velocity), venue_capacity - running_tickets_sold
        )
                ...
                sales_data.append(
                    {
                        ...
                        "cumulative_revenue": running_tickets_sold
                        * average_ticket_price,
                        "sales_rate": round(
                            (running_tickets_sold / venue_capacity) * 100, 2
                        ),
...
def generate_all_ticket_sales():
    ...
    for idx, show_row in future_shows.iterrows():
        print(
            f"Processing {show_row['ARTIST_NAME']} at {show_row['VENUE_NAME']} on {show_row['SHOW_DATE']}"
        )
