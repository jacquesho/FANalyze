...
def _build_show_rows_csv(
    artist_name: str, artist_id: str, setlists: List[Dict[str, Any]]
):
    rows = []
    for sl in setlists:
...
def main(load_to_snowflake: bool = True) -> bool:
    ...
    if not csv_path.exists():
        ...
        console.print(
            "Run scripts/export_setlistfm_history_to_csv.py first.", style="yellow"
        )
        return False
    ...
    if load_to_snowflake:
        ...
        console.print(
            "❄️  Loaded to Snowflake SHOWS_HIS" if ok else "⚠️  Snowflake load failed",
