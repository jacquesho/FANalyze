...
class TicketSalesStreamer:
    def __init__(
        self, speed_multiplier=1.0, output_format="jsonl", save_to_postgres=True
    ):
        """
    ...
    def get_future_shows(self):
        ...
        if not df.empty:
            ...
            print("📊 Real data summary:", file=sys.stderr)
            ...
            print(
                f"   Shows with existing sales: {(df['TICKETS_SOLD'] > 0).sum()}",
                file=sys.stderr,
            )
    ...
    def calculate_sales_velocity(
        self, show_date, artist_tier, venue_capacity, days_until_show
    ):
        ...
        tier_multipliers = {"A-list": 1.5, "B-list": 1.0, "C-list": 0.7}
    ...
    def generate_sale_event(self, show_row):
        ...
        sales_velocity = self.calculate_sales_velocity(
            show_date, artist_tier, venue_capacity, days_until_show
        )

        # Calculate time since last sale (in hours)
        hours_since_last_sale = (
            datetime.now() - sales_tracker["last_sale_time"]
        ).total_seconds() / 3600

        # Probability of a sale happening (based on velocity and time elapsed)
        sale_probability = min(
            sales_velocity * hours_since_last_sale * 24, 0.8
        )  # Cap at 80%

        if random.random() < sale_probability:
            # Generate sale
            tickets_in_sale = random.randint(
                1, min(8, venue_capacity - sales_tracker["tickets_sold"])
            )
    ...
    def run_stream(self, duration_minutes=None, max_events=None):
        ...
        print("Starting real-time ticket sales stream...", file=sys.stderr)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Real-time ticket sales stream generator"
    )
    parser.add_argument(
        "--speed",
        type=float,
    ...
    parser.add_argument(
        "--format",
        choices=["jsonl", "console", "kafka"],
        default="console",
        help="Output format",
    )
    parser.add_argument(
        "--duration", type=int, help="Duration in minutes (None = forever)"
    )
    parser.add_argument(
        "--max-events", type=int, help="Maximum number of events to generate"
    )
    ...
    streamer = TicketSalesStreamer(
        speed_multiplier=args.speed, output_format=args.format
    )

    # Run stream
    streamer.run_stream(duration_minutes=args.duration, max_events=args.max_events)
