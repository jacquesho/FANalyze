...
def generate_show_for_artist(artist, venue, show_date):
    ...
    # Generate IDs
    show_id = f"future_{artist.lower().replace(' ', '_')}_{show_date.strftime('%Y%m%d')}"
...
def main():
    ...
    print(f"🎤 Original artists: {', '.join(sorted(df_existing['artist_name'].unique()))}")
    ...
    print(f"📅 Date range: {df_combined['show_date'].min()} to {df_combined['show_date'].max()}")
