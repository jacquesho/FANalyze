...
def validate_table_structure(table_name="staging.test_ingest"):
    ...
    try:
        ...
        return {"success": True, "columns": columns, "column_count": len(columns)}
...
def display_validation_report(table_name="staging.test_ingest"):
    ...
    if structure_result["success"]:
        ...
        for col in structure_result["columns"]:
            table.add_row(
                str(col[0]), str(col[1]), str(col[2]), str(col[3]) if col[3] else "None"
            )

        console.print(table)
    else:
        console.print(
            f"❌ Table structure validation failed: {structure_result['error']}",
            style="red",
        )
        return False
    ...
    if integrity_result["success"]:
        ...
        null_table.add_row(
            "ID",
            str(null_check["non_null_ids"]),
            "✅" if null_check["non_null_ids"] == null_check["total_records"] else "❌",
        )
        null_table.add_row(
            "Data Content",
            str(null_check["non_null_data_content"]),
            "✅"
            if null_check["non_null_data_content"] == null_check["total_records"]
            else "❌",
        )
        null_table.add_row(
            "File Name",
            str(null_check["non_null_file_names"]),
            "✅"
            if null_check["non_null_file_names"] == null_check["total_records"]
            else "❌",
        )
        null_table.add_row(
            "Loaded At",
            str(null_check["non_null_loaded_at"]),
            "✅"
            if null_check["non_null_loaded_at"] == null_check["total_records"]
            else "❌",
        )
        ...
        console.print(
            f"❌ Data integrity validation failed: {integrity_result['error']}",
            style="red",
        )
        return False
